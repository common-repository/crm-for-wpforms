<?php
    ...
    if ($token && $token != null) {
        ...
        // api with total
        $cloodo_base_url = 'https://erp.cloodo.com/api/v3';
        $api_endpoints = array(
            'lead' => $cloodo_base_url . '/leads',
            'agent' => $cloodo_base_url . '/ticket-agents?limit',
            'client' => $cloodo_base_url . '/clients',
            'proposal' => $cloodo_base_url . '/proposals',
            'employee' => $cloodo_base_url . '/employees'
        );
        ...
        foreach ($api_endpoints as $key => $value) {
            ...
            if ($response_data && is_array($response_data)) { 
                $data_totals[$key] = isset($response_data['meta']) && isset($response_data['meta']['paging']['total']) ? $response_data['meta']['paging']['total'] : 0;
            }
        }
